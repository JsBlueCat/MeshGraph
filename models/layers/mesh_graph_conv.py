...
class NormGraphConv(nn.Module):
    def __init__(self):
        super(NormGraphConv, self).__init__()
        self.graph_conv = SAGEConv(3, 64)
    ...
    def forward(self, x, edge_index, opt):
        result = self.graph_conv(x, edge_index)
        # x = x.view(opt.batch_size, -1, 3)  # n 1024 3
        # result = None
        # for i in range(opt.batch_size):
        #     temp = self.graph_conv(x[i], edge_index)  # 1024  3 -> 1024 64
        #     if i == 0:
        #         result = temp
        #     else:
        #         result = torch.cat([result, temp], dim=0)  # 1024 * i 64
        result = result.view(opt.batch_size, -1,
                             64).transpose(1, 2)  # 64 64 1024
        return self.norm_mlp(result)
